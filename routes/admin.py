"""
routes/admin.py  —  FIXED (removed stale `user` param from all routes)
Old Supabase JWT auth passed a user object; password auth doesn't.
No logic changed — just removed the unused first argument.
"""
import logging
...
@require_admin
def dashboard():
    return jsonify({"message": "Welcome, Admin", "email": "admin"})
...
def stats():
    try:
        ...
        return jsonify({
            "total_students": total, "demo_booked": booked, "enrolled": enrolled,
            "cold_leads": cold_leads, "upcoming_demos": upcoming, "no_shows": no_shows,
            "avg_score": avg_score,
...
def update_status(student_id):
    ...
    try:
        ...
        logger.info("Admin updated student %s → %s", student_id, new_status)
